...
public class SpringRestTempletHandler implements MarkerConstants{
    ...
    public <R> ResponseEntity<R> post(String url, Object body, HttpHeaders headers,RestTemplate restTemplate, Class<R> responseClass) throws ApiHubException {
        try {
            ...
            throw new APICallException("gateway-3002-api","Unexpected REST client failure: " + e.getMessage(), "An unexpected error occurred while trying to communicate with an external service.");
        }
    }
    ...
    private ApiHubException mapToCustomException(ResourceAccessException e) {
        ...
        if (cause instanceof java.net.UnknownHostException) {
            return new NetworkOrTimeoutException("gateway-7005-api", "DNS resolution failed for host. Details: " + originalMessage , "Unable to resolve the address of the external service");
        } else if (cause instanceof java.net.ConnectException) {
            return new NetworkOrTimeoutException("gateway-7003-api","Host unavailable or connection refused. Details: "+ originalMessage, "Could not connect to the external service. It may be down or a firewall is blocking the connection.");
        } else if (cause instanceof java.net.SocketTimeoutException) {
            return new NetworkOrTimeoutException("gateway-7002-api","Request to external service timed out. Details: "+ originalMessage, "The external service did not respond in time.");
        } else if (cause instanceof javax.net.ssl.SSLHandshakeException) {
            return new NetworkOrTimeoutException("gateway-7004-api","SSL handshake failed. Details: "+ originalMessage, "A security error occurred while connecting to the external service (SSL handshake failed).");
        } else if (originalMessage.toLowerCase().contains("timed out")) { // General timeout message check
            return new NetworkOrTimeoutException("gateway-7002-api","Network timeout. Details: "+ originalMessage, "The connection to the external service timed out.");
        } else if (originalMessage.toLowerCase().contains("connection reset")) {
            return new NetworkOrTimeoutException("gateway-7007-api","Connection reset. Details: "+ originalMessage, "The connection to the external service was unexpectedly closed.");
            ...
            return new NetworkOrTimeoutException("gateway-7008-api","Too many redirects. Details: "+ originalMessage, "The request resulted in too many redirects.");
        } else if (originalMessage.toLowerCase().contains("proxy")) {
            return new NetworkOrTimeoutException("gateway-7006-api","Proxy error. Details: " + originalMessage, "An error occurred with the proxy server while trying to reach the external service.");
        }

        // Default network error if no specific cause is matched
        return new NetworkOrTimeoutException("gateway-7001-api","Network unreachable or other network error. Details: "+ originalMessage, "A network error occurred while trying to communicate with an external service.");
    }
    ...
    public ApiHubException wrap(HttpStatusCodeException ex) {
        ...
        if (statusCode == 401) { // Unauthorized
            ...
            return new AuthenticationException("gateway-2001-api", errorMessage, userMessage);
            ...
            return new AuthenticationException("gateway-2006-api", errorMessage, userMessage);
            ...
            return new APICallException("gateway-3002-api", errorMessage, userMessage); // EXTERNAL_SERVICE_FAILURE
        } else if (statusCode == 404) { // Not Found
            userMessage = "External resource not found: The requested resource could not be found on the external service.";
            return new APICallException("gateway-3001-api", errorMessage, userMessage); // API_NOT_FOUND
            ...
            return new InputException("gateway-1010-api", errorMessage, userMessage); // MALFORMED_REQUEST (could also be 1001, 1002 etc. based on body)
            ...
            return new APICallException("gateway-3004-api", errorMessage, userMessage); // API_TIMEOUT
            ...
            return new APICallException("gateway-3008-api", errorMessage, userMessage); // RATE_LIMIT_EXCEEDED
        }
         else {
            ...
            return new APICallException("gateway-3002-api", errorMessage, userMessage); // Generic EXTERNAL_SERVICE_FAILURE
        }
    }
